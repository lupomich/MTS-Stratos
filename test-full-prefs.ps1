...
Write-Host "`nInitial preferences loaded:"
...
Write-Host "`nSaving new preferences..."
...
Write-Host "`nReloaded preferences from DB:"
...
if (($prefs2.preferences.ui_settings.theme -eq 'light') -and ($prefs2.preferences.ui_settings.language -eq 'it')) {
    Write-Host "`nSUCCESS: Preferences persisted correctly in database!"
} else {
    Write-Host "`nFAILED: Preferences not persisted correctly"
}
